// =========================================================
//  CHART GENERATOR — 100% browser (sin node, sin PIL)
//  Reemplaza axios por fetch nativo, PIL por Canvas API
// =========================================================

// ===== CONFIG =====
const BASE_URL = "https://api.tuservicio.com"; // tu base url
const REQUEST_TIMEOUT = 10000; // 10 segundos

const LASTFM_API_KEY = "fe91cc33c93ea3c390251067966de1ba";
const LASTFM_API_SECRET = "3ccee7feee014a82f0c47bd894a8796c";

const SPOTIFY_CLIENT_ID = "01800d9a6c004dc9863ae48feb01af86";
const SPOTIFY_CLIENT_SECRET = "6c6d109a079e4565bdbe9a85befd6edd";
// ⚠️ ojo mostro: tener client_secret expuesto en código de browser es peligroso,
// cualquiera que abra devtools lo puede ver. Esto debería vivir en un backend
// y el browser solo pedirle un token a TU servidor. Te dejo la nota abajo.

const DEFAULT_USER = "pgbito";

const ARTIST_IMAGES_REGEX = /https:\/\/lastfm[.]freetls[.]fastly[.]net\/i\/u\/avatar170s\/(.*)/;
...
const DESIRED_SIZE = 320; // (320, 320) del original en PIL
const DEBUG = false;

// cache de imágenes ya descargadas, igual que tu chart_data global
const chart_data = {};

// =========================================================
//  FETCH HELPERS (reemplaza axiosClient de node)
// =========================================================
...
// equivalente genérico a tu función fetch() de node, pero con fetch nativo
...
// =========================================================
//  SPOTIFY TOKEN (reemplaza spotipy.SpotifyOAuth de python)
// =========================================================
// ⚠️ Esto NO se puede hacer client-credentials flow con secret en el browser
// de forma segura. Lo dejo funcional pero deberías mover esto a un backend
// que te devuelva solo el access_token.
...
// =========================================================
//  LAST.FM CHART REQUEST (reemplaza request_chart de node)
// =========================================================
...
// =========================================================
//  GET IMG (reemplaza get_img de node — ahora sin session raro)
// =========================================================

async function getImg(url, q = null) {
    if (url) {
        // ya viene una url directa (típico de last.fm), la devolvemos tal cual
        // para que la use el <img> / canvas más adelante
        return url;
    }
...
// mantiene la misma firma [artist, url] que usaba el resto del código
...
// =========================================================
//  PLAN B — si no querés depender de Spotify: proxy propio
// =========================================================
// si en algún momento SÍ querés las fotos reales de last.fm (a veces
// tienen mejores fotos de banda que Spotify para artistas under­ground),
// la única forma es un backend tuyo que haga el scraping y te lo sirva
// con headers de CORS abiertos. Server chiquito en Node con Express:
//
//   app.get("/artist-image", async (req, res) => {
//     const artist = req.query.artist;
//     const html = await fetch(
//       `https://www.last.fm/music/+noredirect/${encodeURIComponent(artist)}/+images`
//     ).then(r => r.text());
//     const match = /https:\/\/lastfm[.]freetls[.]fastly[.]net\/i\/u\/avatar170s\/(.*?)["']/.exec(html);
//     const id = match?.[1];
//     res.set("Access-Control-Allow-Origin", "*"); // clave: esto habilita el fetch desde tu browser
//     res.json({
//       url: id
//         ? `https://lastfm.freetls.fastly.net/i/u/770x0/${id}.jpg`
//         : null,
//     });
//   });
//
// y en el browser, en vez de scrapear directo, le pegás a TU endpoint:
//
//   async function scrapeArtistImagesForChart(artist) {
//     const res = await fetchWithTimeout(
//       `https://tu-backend.com/artist-image?artist=${encodeURIComponent(artist)}`
//     );
//     const data = await res.json();
//     return [artist, data.url || NO_IMAGE_PLACEHOLDER];
//   }
//
// esto SÍ te da las fotos reales de last.fm, pero implica mantener un
// servidorcito corriendo en algún lado (Render, Railway, un VPS, etc).
...
// =========================================================
//  IMAGE / CANVAS — traducción de las funciones PIL
// =========================================================
...
function loadImage(src) {
    return new Promise((resolve, reject) => {
        ...
        if (src instanceof Blob) {
            ...
            img.src = src; // string url
        }
    });
}

// equivalente a textwrap.wrap() de python
...
// canvas no tiene stroke nativo en fillText, se simula con strokeText + fillText
...
// equivalente a process_item() de PIL
// item = [atextOriginal, byteitemOrUrl, needsResize]
async function processItem(item) {
    ...
    // texto (equivalente a draw.text con stroke_width=1)
    let height = 25;
    let line0 = atext[0];
...
// =========================================================
//  LAST.FM — top artists / top albums (reemplaza tus funciones
//  "userGetTopArtists" / "userGetTopAlbums" que llamaban a la lib
//  python "last" desde JS, cosa que no existe — las hice con fetch)
// =========================================================
...
// =========================================================
//
// const blob = await requestChart("pgbito", "albums", "overall", 3, 3);
